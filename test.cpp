...
void displayAllLibrarians(){
	...
	DisplayAllFile.open("librarian.dat", ios::in);
	string line;
	
	while(getline(DisplayAllFile,line)){
		istringstream iss(line);
		string id,fullName,email,conNumber,address;
		getline(iss,id,'|');
		getline(iss,fullName,'|');
		getline(iss,email,'|');
		getline(iss,conNumber,'|');
		getline(iss,address,'|');
		
		cout << "\t\tLIBRARIAN ID\t:" << id << endl;
		cout << "\t\tFULL NAME\t:" << fullName << endl;
		cout << "\t\tEMAIL\t\t:" << email << endl;
		cout << "\t\tCONTACT NUMBER\t:" << conNumber << endl;
...
void issueReturnedBooks(){
	...
	while(true){
		...
		if(bookFound==1 && borrowerFound==1){
